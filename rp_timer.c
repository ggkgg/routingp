...
{RP_TDELAY_S, RP_TDELAY_US},    //1s,0us  ,   this is timer interval(space)
{RP_TINTVL_S, RP_TINTVL_US}     //0s,200000us , this is timer begin
};
...
void rp_tsch_init()
{
	...
	for (i = 0; i < rp_tfs; i++)
	{
		...
		//等待cnt个系统定时器周期激活一次定时器rp_tsch.procs[i]
...
void rp_sop_gen(void *data)
{
	...
	tmsg.node = *sa;
#else
	tmsg.node = MADR_BRDCAST;//(MADR)0xFFFFFFFF
#endif
	/*rp message header */
